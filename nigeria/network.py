...
if __name__=="__main__":
	...
	## Get the data
	train, valid, test = load_data.PortData(y=['h0'])
	#test = valid + test
	print("Number of training data points is",len(train))
	print("Number of validation points is",len(valid))
	print("Number of test data points is",len(test))

	## Network size stuff
	input_layer = train[0][0].shape[0]
	network_geometry = [input_layer,100,50,2]
	print("Network geometry =",network_geometry)

	## Train the model
	model = NeuralNetwork(network_geometry)
	model.sgd_training(train, epochs=100, mini_batch_size=100, 
					  learning_rate=0.4, weight_decay=4.,valid_data=valid)

	## Try it on the test data
	test_correct = model.evaluate(test)
	print("Model performance on the test data = {0}/{1} ~ {2} %".format(test_correct,len(test),(100.*test_correct)/len(test)))

	## Save the model
	model.save('h8_network.pkl')
